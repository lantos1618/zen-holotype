...
# File: fd(7)+mid(0)*100+end(1)*10 = 17. (Examples 1+2 of zen/std/cown.zen, flattened.)
...
VERDICT_CASES = [
    ...
    # (genc's ret_to_expr) makes the block's TRAILING statement the yielded value, but an early
    # `return` mid-block is turned into a bare `e;` and silently dropped — control never leaves the
    # function and the WRONG (later) expr is yielded. Guard returns must be STATEMENT-position
