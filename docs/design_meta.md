...
**⚠️ THIS SECTION IS SUPERSEDED.** The bootstrapper is deleted. The old
constraint — *`src/` may not use `@meta`, because the bootstrapper would have
to compile it* — no longer holds. The live constraint is that an `@meta` in
`src/` must be expandable by the compiler built from `seed/zen.c`. Land the
expander first, regenerate the seed, then land its first use.
...
| but it was **on the verification path for `src/`** | no longer: stage 0 is `cc seed/zen.c`; the bootstrap and its Python parser are deleted |
...
The live grammar parses both `@meta` forms; the compiler owns their AST and semantics.
...
| the grammar's twin | `grammar/grammar.js:591` — so `make parse` covers the syntax |
...
**Decision complete.** The table above went to zero rows: R1–R4 answer the four rows, R5 settles A-META-ARG, and R6 settles the two forms.
...
**Skeleton landed under M3, as §7.2 said it would** — and the skeleton is what landed, not the evaluator. The budget existed from the first commit and REPORTED: `META_STEPS: usize = 4096`, threaded as remaining fuel, raising `SemaFault.ComptimeBudget`. **Removed 2026-08-24 (#768):** three spends on a straight line cannot reach zero, so the counter could never fire — and it was not charged into the one callee that recurses (`type_from_ast`), so it did not bound the thing it existed to bound either. A gate that cannot fail reads as coverage. It returns with the first rule that can loop (M2, or M4's nested-struct recursion), together with the must-fail test §6.1 owes it, and it must be charged into whatever can recurse when it does. Still M2's, all of it: comptime loops, allocation, and the known/residual value domain — `.name` needed none of §5's machinery, and staging arrives with `self.at(field)` and `fields()` in M4.

### M3 — READ, one member deep. LANDED
`@meta(name: Type).name` folds at sema to the WRITTEN type's declared name, a real `str` — the typed form with `@Self` inside a struct/impl body, and with any named type (`@meta(s: Shape).name`); Struct, Enum and Alias payloads fold their `name.text`. An alias folds to what the user WROTE (`P2 = Point` … `@meta(x: P2).name` answers `"P2"` — resolved through `sema_denote.written_type`, one step short of the alias collapse; #768), and an alias whose target has no declaration payload (`Count = i64`) is refused like any other payload-less type. **The fold is memoised keyed on the written TYPE EXPRESSION, not the access node (#768):** two spellings are two rows even where their answers agree, which is also what keeps two instantiations of one generic apart — today decl-level names make them agree anyway, but the key is what M4's instantiation-dependent field walk will need, and it is laid down before anything can depend on the collision instead. The backend reads the fold back through the access's own `Meta` node (`gen_c_expr.lower_meta_or_access`) and emits the ordinary string-literal shape — the backend learned nothing new. Everything else still refuses with `MetaNotImplemented`, one diagnostic at the `@meta` token: value form, bare typed form, unknown member, primitives. **Also landed:** the M2 skeleton (its budget removed, see above). **Gate:** the `corpus/meta/` pair `type_name_via_meta` + `type_name_by_hand`, one shared `.expected`; plus `corpus/meta/alias_name_is_what_was_written` (P2/Point) and `corpus/meta/alias_with_no_payload_refused` (Count = i64), both #768's observable halves. **Risk retired:** the fold went through `sema_meta.zen` and the Checker's memo, not a backend special-case.

### M4 — STAGING: the field walk
`.fields().loop((h, field) { .. })` unrolled once per field, `field.name` known, the instance projection residual (§4, §5). **Unblocks all five waiting consumers**, one lane each: `Display.dump` (`std/core/display.zen:33`), the `Eq` default (`std/core/eq.zen:10`), the `Hash` default (`std/core/hash.zen:31`), `Tester.expect_eq` (`std/test/test.zen:37`), `Env.args`' schema fill (`std/env/env.zen:153`). **Gate:** hand-written twins, plus an expectation that names every field with a distinct non-default value and asserts the count (§6.4). **Risk:** this is the keystone and the largest single piece. A skipped field prints plausibly; a nested struct recurses into `dump` and is where a step budget first earns its keep; and hygiene — `gen_c_inline.zen`'s header is the record of how a captured `h` goes silently wrong rather than loudly. **Not working today, recorded so M4 is not planned as though it does (#768):** reflection THROUGH a generic parameter — `nameof<T> = (v: T) str { @meta(x: T).name }` — is refused: the written `T` resolves to the enclosing function's type-parameter variable, which carries no declaration payload. Whether that form means "the instantiation the caller made" is an M4 decision (§5.2: a body containing `@meta` is the first body for which check-once is wrong), not something already answered.

**Slice 1 landed:** the count. `@meta(name: Type).fields().len` folds at sema to the written type's field count — `Struct.members` filtered to `MemberKind.Field`, so a constant and a method do not count — memoised on the written TYPE EXPRESSION for the same reason `.name` is, and lowered as an ordinary integer literal (`gen_c_expr.lower_meta_count`). The chain is intercepted at its top (the `.len`), strictly syntactically (`sema_meta.is_count_chain`); a bare `@meta(T).fields()` stays the wall. **Gate:** `corpus/meta/field_count_via_meta`.

**Slice 2 landed 2026-08-28:** the walk itself. `@meta(name: T).fields().loop((h, field) { .. })` is checked ONCE PER FIELD at sema (`sema_meta.meta_walk_chain`) with the two binders bound and recognised BY THEIR DECLARATION — a body that rebinds `field` or a nested loop that rebinds `h` shadows the walk's meaning (`corpus/meta/field_walk_shadowing`). `field.name` folds to the pass's name as a comptime `str`; `recv.at(field)` types as the pass's field and stays RESIDUAL — the backend rewrites it to the ordinary `recv.<name>` (`gen_c_meta.lower_meta_proj`) and unrolls one block per field (`gen_c_meta.lower_meta_walk`), with `h` an ordinary handle over the whole walk: `break()` leaves it, `next()` leaves the pass. The per-pass answers travel two ways: the memo holds the LAST pass's rows, and while the backend lowers a pass it pushes `Checker.walk_now`, which the projection's type (`sema_meta.walk_projection_type`, asked through the one `type_of` funnel so generic instantiation still applies after it) and the `field.name` fold both read. Two deliberate edges, both the wall rather than a guess: a NESTED walk is refused — it is the recursion §8's missing budget exists for, so the shape is declined and no budget is owed — and a body ERROR is reported once per field (the must-fails walk one-field structs, so their counts stay 1). Also walled: `h.break(v)` (the walk has no value; M4-later), either binder standing as a value, a walk over a type parameter or other payload-less type, and a projection the receiver's type does not carry (a `NoSuchField` naming the comptime field — `must-fail/sema/meta_projection_receiver_mismatch`). **Known hole, M4-later:** an UNMARKED compound node containing the projection (`self.at(field) + 1` over mixed-numeric fields) keeps the last pass's memo types for the backend's type-directed lowering — sema still checks every pass, so diagnostics are right; the fix is §5.2's memo re-keyed on (ExprId, instantiation). **Gate:** the `corpus/meta/field_walk_via_meta` + `field_walk_by_hand` twin pair sharing one `.expected`, plus `field_walk_shadowing`; refusals pinned by five must-fails (`meta_nested_walk_refused`, `meta_field_as_value_refused`, `meta_walk_over_type_parameter_refused`, `meta_walk_break_with_value_refused`, `meta_projection_receiver_mismatch`) and per-pass checking by `meta_walk_checks_every_field` (clean in pass one, a mismatch in pass two — a memo-reading checker compiles it). **Still open within M4:** the five consumers, `h.break(v)`, and the §5.2 re-key.
...
- **How to MEASURE the step budget's number — and whether there is one at all.** There was an answer in code, `META_STEPS: usize = 4096`, documented as a setting, not a measurement; #768 removed the counter because three straight-line spends could never reach zero. The number, the per-`@meta`-call vs per-compilation question, AND the counter itself all get their real answer with the first rule that can loop (M2, or M4's nested-struct recursion) — which must also charge whatever can recurse, or the next review finds the same hole. M4 slice 2 met the nested shape first and DECLINED it: a walk inside a walk's body is the wall (`must-fail/sema/meta_nested_walk_refused`), so the walk unrolls a known field list and never recurses — the budget is still owed only by the first rule that can actually loop.
- **The budget must-fail test is owed, and with the counter gone the COUNTER is owed first.** §6.1's rule — write the budget test the same day the budget lands — stands; #768 removed the budget rather than leave a gate that cannot fail reading as coverage, so the test lands with the counter's return, against a rule that can actually spend the fuel down.
