...
**⚠️ The residue's cause is deleted; the residue has not been re-measured.** All 13 names below were owed to the Python bootstrapper, which is gone (`PLAN.md`, "Retiring the bootstrapper"), and `make fixpoint` no longer runs it. So `IMPORT_OWED` is very likely now a debt to nobody — but "likely" is not a measurement, and the honest close is to delete the 13 import lines, run `make test` and `make fixpoint`, and shrink the ledger by what stays green. Until somebody does that, the paragraphs below are the record of why they were added.

The residue is 13 names in `IMPORT_OWED`, and they were all one defect, whose shape is narrower than it had previously been written down as. **The bootstrapper resolved a method reached through a FIELD by the field's type NAME as the importing file spells it — but only when that method name had a competitor reachable in the same compilation.** `c.types.at(..)` needs nothing: `lsp_def.zen` and `lsp_compl.zen` write it with no `Types` imported, and both carried a comment claiming otherwise which this change deleted. `c.types.write_name(..)` needs `Types`, because `sema_diag.zen:336` declares a *free function* of that name and the bootstrapper picks it — `write_name is not exported by module sema_diag`. All six method names in the residue have such a twin: `bind` in `parse_stmt.zen`, `count` in `gen_name.zen`, `index_of` in `collections_map`, `bump` in `mem_arena`, and `fn_ty` and `count` again in the corpus tests' own `main.zen`.

Which names were load-bearing therefore depended on the compilation, which is why the 13 were found by running gates rather than by reading code. Three failed only under `make fixpoint`, whose stage 1 bootstrapped all of `src/`; eight failed only under `corpus/sema_zen` and `corpus/gen_zen`, where the competitor is in the test program's own main. `make build` was blind to every one of them — **the self-hosted compiler resolves them all correctly**, which is the sentence that matters now, since it is the only compiler left. The mechanical form of the rule, restoring the type of every intermediate field receiver, kept 145 names, which was 132 import edges no gate could justify. Each of the 13 carries its receiver chain in a comment on its own import line.
