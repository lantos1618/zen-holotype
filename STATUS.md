...
Last code/document audit: 2026-07-12, from the self-hosted tree based on `5f33f01`.
...
| One understandable language structure | Records, enums, signatures, traits-as-records, UFCS, and value matches form a promising core. Guard `if`, enum/bitwise `\|`, internal `@while`, evaluate-more-than-once lowering, and backend name special cases still weaken it. | **Partial — coherent direction, unsafe lowering edges** |
...
| Records, enums, exhaustive match, guards, loops | Partial: some subjects repeat and statement/JS guards can disappear | `parse*`, `check`, emitters | value/verdict/formatter fixtures | Strong |
...
of harness source. This audit removed two byte-duplicate fixtures, four misleading
`bool_guard_wild` fixtures that contained no wildcard, a committed generated ELF, and stale
hard-coded verdict case counts. It also wired two valuable fixtures that previously had no harness
reference (fixed-width C/JS parity and `Vec.view().loop`).
...
|---|---|---|
| Ordinary `if` | Rejected with targeted `error[if-statement]`. Match-arm guards still use `pattern if condition`. | Use `.match`; migrate guards to `pattern when condition` so `if` disappears from ordinary source. |
...
| P0 | Match guards are silently discarded in statement-position C lowering and JS switch lowering. | `compiler.check::lower_match_arms`; `compiler.genjs::js_match_*` | A false first guard falls through on C and JS; only then migrate guard spelling from `if` to `when`. |
...
| P0 | Parameter-capturing closure substitution ignores lambda-local shadowing. Capture analysis also omits match guards. | `compiler.check::clos_sub_var`, `clos_sub_stmt`, `lift_arms_cap`, `clos_bad_arms` | Local-shadow and guard-only-capture closures preserve lexical binding. |
...
| P0-B | Evaluate-once and hygienic lowering | Ordinary expressions can currently run twice, vanish, lose a guard, or collide with compiler temporary names. This is silent miscompilation in the language core. | Compound assignment, every match form, generic substitution, and closure lowering preserve source evaluation exactly once; guards survive; all compiler names are hygienic. |
...
| P1 | Guard spelling migration | Once guards actually work in every lowering, `pattern if condition` remains the only source-level exception to “no `if`.” | Parser/formatter/tests use `pattern when condition`; ordinary `if` keeps the targeted `.match` diagnostic; old guard spelling is removed after bootstrap migration. |
