...
| `tests/atomic_test.zen` | Unwired and always returns success after printing. | Done: replaced by `tests/fixtures/zen/atomic_wrappers.zen` (asserts each check, exits 42 on success, N on the failing group) and wired as a build-fixture case. |
...
| P1 | Top-level decl parsing is one recursive call per decl, so a module near the 16,384-`decl_buf` cap dies with a stack-overflow panic instead of a diagnostic — and it masks the registry-capacity rejection (`error[capacity]`, code 47), which is only reachable below that ceiling today. | `compiler.parse::fill_decls` and its ~10 mutually tail-recursive one-decl helpers | Modules at/above the decl cap get a clean positioned diagnostic; `error[capacity]` fires at registry limits. Convert the tail recursion to an explicit loop. |
