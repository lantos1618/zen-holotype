...
| file          | what it is                                                            |
|---------------|-----------------------------------------------------------------------|
| `zenc.gen.c`  | the four `.zen` sources, compiled to C by the toolchain itself (generated) |
| `zenrt.h/.c`  | a ~30-line runtime: the growable `String`, `eq`/`is_empty`, `heap`     |
| `main.c`      | the CLI entry — reads Zen source (file arg or stdin), writes C to stdout |
| `generate.py` | regenerates `zenc.gen.c` from `std/*.zen` (the only Python, build-time) |
