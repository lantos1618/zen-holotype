...
| P1 | Top-level decl parsing is one recursive call per decl, so a module near the 16,384-`decl_buf` cap dies with a stack-overflow panic instead of a diagnostic — and it masks the registry-capacity rejection (`error[capacity]`, code 47), which is only reachable below that ceiling today. | `compiler.parse::fill_decls` and its mutually tail-recursive one-decl helpers | Modules at/above the decl cap get a clean positioned diagnostic; `error[capacity]` fires at registry limits. Convert the tail recursion to an explicit loop. |
