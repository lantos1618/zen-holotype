...
KNOWN_SUFFIXES = {
    ".zen", ".expected", ".exit", ".stderr", ".count", ".stage",
    ".stdin", ".args", ".env",
}
...
RULES: dict[str, tuple[str, str]] = {
    ...
    "orphan-aux": (ERROR, "a test sidecar with no test beside it"),
    ...
    "mf-aux-file": (ERROR, "a must-fail test carries a runtime sidecar, which the format does not define"),
...
class Linter:
    ...
    def check_flat_file(self, kind: str, base: Path, f: Path, suite: str) -> None:
        ...
        if f.suffix == ".zen":
            ...
            if kind == "must-fail":
                ...
                for aux in (".exit", ".stderr", ".stdin", ".args", ".env"):
    ...
    def check_dir_test(
        ...
        if kind == "must-fail":
            ...
            for name in (
                ".exit", ".stderr", ".stdin", ".args", ".env",
                f"{d.name}.exit", f"{d.name}.stderr", f"{d.name}.stdin",
                f"{d.name}.args", f"{d.name}.env",
                "main.exit", "main.stderr", "main.stdin", "main.args",
                "main.env",
            ):
                if (d / name).is_file():
