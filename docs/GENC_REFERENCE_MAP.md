...
```python
def generate(program, sema=None, root=None, sources=None):      # gen_c.py:5709
...
Returns `(C source text: str, diagnostics: tuple)`. Diagnostics are de-duplicated on `(span, message)` — "a type resolved for a prototype and again for a body is one bug" (5717-5719). Never raises; `Emitter.error` (1586) appends `ast.Diag`.

`program` is duck-typed: `modules_of` (522) accepts a `modules.ModuleGraph` (has `.modules` dict + `.lookup`), a dict, a list, or a bare `ast.Module`. `self.graph` is set only when the object has both `.modules` and `.lookup` (745-747).

## 1.2 Top-level names

Constants/tables: `C_STANDARD` (157), `USR="zu_"` / `GEN="zg_"` (159-160), `MAX_INSTANCES=4096`, `MAX_EXPR_DEPTH=24`, `INLINE_DEPTH=32` (162-164), `ENVMARK="\x01env\x01"` (169), `PRIMS` (285), `INT_LIMITS` (303), `INT_VALUES` (317), `NUMERIC` (330), `UNIT`/`UNKNOWN`/`INFER` (332-339), `ARITH`/`WRAPPING`/`COMPARE` (5316-5318), `_LVALUE` (5312), `_IDENT` (200), `_ESCAPES` (5379).

Free functions: `_sibling` (117), `kind` (181), `f` (185), `_diag` (189), mangling `comp/clist/path_code/sym_type/tcode_named/sym_fn/sym_variant/sym_member/sym_local/sym_value` (203-265), `prim` (342), `tcode` (346), `is_int` (368), `int_info` (372), `union_of` (376), `_skip_trivia` (448), `modules_of` (522), `module_parts` (554), `_tparam_names` (603), `_is_variadic` (607), `_bare_name` (613), `_type_args` (618), `_block_value` (627), `_bound_apps` (642), `_bound_names` (663), `_is_fn_field` (687), `is_loop_shape` (5228), `erase` (5250), `refine` (5270), `is_handle` (5288), `_writes_scope` (5292), `paren` (5326), `parse_int` (5332), `int_literal` (5348), `float_literal` (5370), `decode_str` (5390), `decode_char` (5420), `c_string` (5428), `_checked_helpers` (5596), `generate` (5709).

Classes: `SourceMap` (405), `Positions` (469), `Decl` (568), `Emitter` (712), `FnCtx` (2057).

Runtime string constants: `BANNER` (5451), `INCLUDES` (5470), `PRELUDE_TYPES` (5480), `PRELUDE_TRAP` (5494), `PRELUDE_SCOPE` (5535), `DEFER_RUNTIME` (5547), `PRELUDE_PRINT` (5582), `HELPERS = _checked_helpers()` (5701).

## 1.3 The two-phase pipeline

`Emitter.__init__` (713-748) builds the declaration tables via `_collect()` (757); then:
...
Whole-program, demand-driven from `main` only (1593-1596) — an unreachable std member never has to compile. `drain` (1607) pops a **sorted** worklist:
...
```
with a `MAX_INSTANCES` guard that reports "generic instantiation did not terminate at `<name>`" (1611-1617).

## 1.4 Output section order — exact

```python
def assemble(self):                                              # gen_c.py:1844
...
1. `BANNER` comment (5451)
2. `INCLUDES` — 6 headers (5470)
3. `prelude()` (1864): `PRELUDE_TYPES` + `PRELUDE_TRAP`, then `PRELUDE_SCOPE` iff `"scope" in self.needs`, then `PRELUDE_PRINT` iff `"print" in self.needs`, then `HELPERS[name] for name in sorted(self.helpers)`
4. `emit_types()` (1896): `/* ---- types ---- */`, then **all** `typedef struct X X;` forward decls (`for cname in sorted(self.types)`), blank line, then one program-wide `enum { ... }` of variant tags (`for name in sorted(self.consts)`), then struct/union bodies in `topo()` order
5. `defer_section()` (1874): capture structs `zg_envN`, the `zg_defer_env` union, then `DEFER_RUNTIME` — emitted only if `"scope" in self.needs`
...
Note `sym_value` is declared but **never called** — module-level bindings are inlined at their use site (`ex_Path` 2506-2527), which is exactly what the docstring at 58-61 reserves it for.
...
|---|---|---|
| named type / monomorphised instance | `USR + tcode(t)` | `request_type` 1082-1085 |
| structural type (array/union/fn) | `GEN + tcode(t)` | `request_type` 1085 |
| function / method / instantiation | `sym_fn(decl.parts, sig, targs, self_ty)` | `request_fn` 1629; `entry_point` 2012 |
| enum constant (tag) | `sym_variant(parts + (vname,))` | `_request_named` 1165, `make_variant` 3530, `pat_conds` 4885/4902, `match_enum` 4940, `ex_Try` 5034 |
| struct member / enum payload member | `sym_member(name)` | `emit_type` 1958/1979, field read 2571, `construct_type` 2998, `make_variant` 3541 |
| trait slot | `sym_member(name) + ("" if seen[name]==1 else "_%d" % seen[name])` | `trait_slots` 1263 |
| local / parameter | `sym_local(name, n)` with a **per-function** counter | `FnCtx.declare` 2117-2124 |
| struct tag | the *same string* as the typedef | `emit_types` 1910 `typedef struct %s %s;` |
| label | `GEN + stem + tmpcounter` | `FnCtx.label` 3635-3637 |
| temporary | `"%st%d" % (GEN, self.tmp)` | `new_tmp` 2102-2107 |
...
## 3.1 Trap runtime (`PRELUDE_TRAP`, gen_c.py:5494-5533) — verbatim
...
**Binary arithmetic** — `fold_binary` (2868-2907). Note that the checked result is **always spilled into a temporary**:
...
**Unary minus** — `ex_Unary` (2769-2793). Negative literals are folded (no trap); a runtime negation is `0 - x`:
```python
if is_int(ty):
...
**Fixed-array bounds check** — `ex_Index` (2736-2754):
```python
...
```
i.e. `arr.zg_elems[zg_idx_u(i, 8, "m.zen", 3, 9)]`. A **`Ptr<T>` index is unchecked** — `("%s[%s]" % (paren(bcode), icode), bty[1])` (2755-2758) — "a raw pointer carries no length ... and `Vec.get` returns a Res instead". A named type's `s[i]` becomes a call to its own `index` method (2759-2765).

`self.e.helpers` is a `set`, drained sorted at `prelude()` 1870. `HELPERS["idx_u"]`/`["idx_s"]` are `""` because those two live in `PRELUDE_TRAP` unconditionally (5696-5697).

**Unreachable match arm** — `match_enum` (4986-4991) when there is no wildcard arm:
```python
file, line, col = self.e.pos.of(node)
...
`Positions.operator` (493-514): if the node has an `op_span`, use it; otherwise scan the source text **between** the left operand's end and the right operand's start, skipping whitespace, `//` and `/* */` (`_skip_trivia`, 448-466), and take the first real byte's `(line, col)`. Fallback is `self.of(node)`. Source text comes from `SourceMap` (405-445), which reads from `root` on disk or from the `sources` dict. File names are made relative to `root` and `\`-normalised to `/` (`relfile`, 476-484), so no absolute path reaches the output. `__FILE__` is never emitted.
...
```
The last two are FnCtx-internal binding markers, never types (`bind_closure` 3648, `lower_loop` 3862).
...
`resolve_type` (932) special-cases: `Ptr`/`RawPtr` → `("ptr", arg or u8)`; `...` → `("variadic",)`; any name in `NUMERIC` → `prim(name)` **even though std declares `i32` etc. as struct bodies** carrying `MIN`/`MAX`/`BITS` (950-956); `@Self` → `self_ty` or the enclosing type; `Alias` transparently resolves to its target.
...
- **`str`** — `zg_str` (`typedef struct zg_str { unsigned char *data; size_t len; }`, 5483) *unless* std declares a `str` type, in which case the user's struct wins and its members are read through `sym_member` (`str_names`/`str_literal` 2478-2496). `str_names` hardcodes the member names `"data"` and `"len"`.
- **Function type** → `zg_closure` (`{ void *fn; void *env; }`, 5487). This is essentially vestigial: closures are inlined, never materialised (`ex_Lambda` 5178 errors).
- **`Scope`** → `zg_scope *` (pointer to the enclosing block's record).
- **`()`** → `void`.
- Everything nominal/array/union → `request_type` mangled name.

`declarator(t, name)` (1071): pointer spellings bind to the name (`int32_t *x`), everything else gets a space. `fnptr(ret, params, name)` (1063): `ret (*name)(void *, params..)` — the receiver is always erased to `void *`, one slot per method serves every impl.

## 4.4 Emitted layouts (`emit_type`, 1946-2001)

**struct**:
```c
struct zu_t...Point { int32_t zu_m1x; int32_t zu_m1y; };
```
An empty struct gets `char zg_pad;` (C11 6.7.2.1p1 requires a member) — "one byte, in exactly one place, so a bare Empty and an Empty field agree" (1953-1956). Function-typed fields are **skipped** — "a method is a function, not storage" (1136-1137).
...
```
The union is omitted entirely when no variant carries a payload. Payload member is named after the **variant**, `sym_member(vname)`. A `()` payload becomes `None` — "`Ok(())` carries nothing; a void member is illegal C" (1158-1159).

Tag constants are one **program-wide** C enum, emitted at `emit_types` 1912-1916, keyed by declaration not instantiation (`_request_named` 1161-1165):
```python
...
**trait (fat value)** — a struct that declares fn-fields and **no storage** (`trait_decl`, 1211-1221):
```c
struct zu_t..Display {
    void *zg_self;
...
```
Slot order is declaration order (`trait_slots`, 1243-1288); duplicate names get `_2`, `_3`. Slot parameter/return types are **erased** (`erase`, 5250-5267: `Ptr<T>` → `Ptr<u8>`, named type args erased recursively), and **one trailing `size_t` per erased type parameter of the member** is appended (1283-1284) — the element size the erasure dropped, Zig-`std.mem.Allocator` style.
...
- `type_decl(name, parts, arity)` (887-906) picks by `len(d.tparams) == arity`, falling back to an `Alias`, then to `got[0]`.
- `_request_named` (1102-1111) picks the candidate whose `len(d.tparams) == len(args)`.
- `filled_ret` (1674-1702) rewrites a declared `Res<T, _>` into `Res<T, <inferred set>>` by asking `sema.error_set_ast(fnode)`, memoised on `id(fnode)` — "the one funnel every caller reads a return type through, so a caller and a callee can never disagree about what `_` turned out to be".

Their C names differ because `tcode` includes the argument list: `zu_t3_3std4core6result3ResI1_b3i32` vs `...ResI2_b3i32<E>`.

`Vec`/`Map`/`String` are **not** special-cased: they are ordinary std structs, monomorphised through `request_type`. The only container the compiler knows intrinsically is the fixed array `[T, N]` (which "satisfies Range intrinsically" — `range_bounds` 4305-4313).

Coercion `T` → `Res<T>` happens in `coerce` (2289-2310) and fires **only when exactly one variant carries that payload type**:
```python
...
## 4.6 Building a fat value (`fat_value`, 1545-1582)
...
`convert` (1419-1451) **rebuilds** — pointers are cast, tagged unions are reconstructed variant by variant as a nested conditional; nothing reinterprets.
...
- `c_string` (5428) emits **octal** escapes, not hex — "which also keeps the determinism scanner's pointer-shaped-hex check quiet" (5431).
...
```python
def expr(self, node, want=None):                                # gen_c.py:2426
...
`new_tmp` (2102) declares at the current indent and returns the name; it declares **nothing** for `UNIT`/`None` types but still burns a counter value.
...
```python
def ex_Block(self, node, want=None):                            # gen_c.py:4720
...
So a block-as-value becomes `T zg_tN; { ...stmts...; zg_tN = <tail>; }` and the expression text is `zg_tN`. A block is lowered **twice** when its type is unknown — `peek_block` (3044-3060) lowers into a buffer that is discarded:
```python
...
`_block_body` (2241-2287) is where the tail rules live:
...
`block_value` (2210-2223) wraps it with the RAII/defer scope stacks (`dscopes`, `sscopes`) and returns the value expression or `None`.

## 6.3 `.match` in value position

```python
def ex_Match(self, node, want=None):                            # gen_c.py:4747
...
The scrutinee is **always** spilled to a temporary first (so it is evaluated once). The result is a temporary; each arm assigns it (`arm_body` 4823-4835). Three lowerings:
...
`arm_body` (4823) handles the void case: `elif result is None and value and value not in ("", "0"): self.line("(void)(%s);" % value)` — "an arm whose body is a call returning `()` still has to RUN".

`arm_type` (4770-4798) computes the match's type when nothing expected one: every arm is peeked (`peek_block` for a Block, `peek` otherwise); **literal arms only contribute a fallback**; non-literal arms are merged with `wider_arm`. `wider_arm` (4800-4821) implements the *only* widening: `Res<T>` vs `Res<T,E>` → the two-arg one; `Res<T,E1>` vs `Res<T,E2>` → `Res<T, union_of([E1, E2])>`.

Pattern matching itself: `pat_conds` (4867-4906) recurses to arbitrary depth, producing a `conds` list and a `binds` list. A bare string payload name is a **variant test** if the payload type declares such a variant and a **binding** otherwise (`Left(Blank)` vs `Left(cell)`, 4876-4888).

## 6.4 `.try()`

```python
def ex_Try(self, node, want=None):                              # gen_c.py:5018
...
`emit_propagate` (5042-5079) first `self.unwind_to(0)` — "every early exit is a scope exit", and the error lives in a *temporary*, which is never dropped, so the whole frame can unwind before the return is built. Then it picks the target variant (`Err`, then `None`, then any non-Ok name), and widens the payload with `widen_error`. When widening returns `None`:
```python
...
```
an ordinary generic function with a body and a closure parameter. It reaches `emit_call` (3545), which at 3575 routes it to inlining:
```python
...
```
`refine(ty, want)` (5270-5285) is what fills `then`'s `T` from the call site when the closure body says nothing — its docstring names `then<T>` explicitly.
...
- `ex_Lambda` (5178-5188) is a hard error: "a closure here would have to escape its frame; an escaping closure needs an Alloc (DESIGN.md)".
- `bind_closure` (3639-3649) binds a parameter name to a marker, **carrying the frame it was written in**:
```python
self.scopes[-1][name] = (name, ("lambda", lam, pty, len(self.scopes),
                                (self.subst, self.parts, self.self_ty)))
```
- `inline_call` (3651-3708): resolves type args, `result = new_tmp(ret)`, pushes a scope, binds the receiver to `params[0]`'s name, evaluates non-lambda arguments into declared locals (with `coerce`, so an Arena reaching an `alloc: Alloc` parameter builds the fat record), binds lambda arguments via `bind_closure`, swaps `(subst, parts, self_ty)` to the callee's, emits `{ ... }` around `block_value(body, ret)`, assigns `result`, restores. Guarded by `INLINE_DEPTH` (3653-3655).
- `inline_lambda` (3710-3770): the arguments are lowered **at the call site** *before* the frame moves; then
```python
home = None
if len(marker) > 4:
...
- The **one** closure that becomes a real C function is a `defer` closure: `Emitter.defer_thunk` (1770-1817). Its body is lowered *before* its capture struct is named, using the `ENVMARK = "\x01env\x01"` placeholder, so two lowerings of one closure compare equal and dedupe through `_defer_cache`; the name is patched in with `.replace(ENVMARK, env or "void")` at 1814-1815. Captures are computed by `captures` (4464-4498) walking the lambda for `Path` nodes that resolve in this frame, excluding names bound inside it, and excluding `lambda`/`loop`/`fn`-typed bindings.

## 6.7 The loop family

`is_loop_shape` (5228-5247) recognises a loop **by shape**, not by module/name: a bodyless function whose last parameter is an `FnType` whose first inner parameter's type is `Named("LoopHandle")`. `emit_call` 3551 routes those to `lower_loop`.

`lower_loop` (3774-3887) — full lowering:

- Inner parameter *names* decide the shape: `wants_index = "index" in names[1:]`, `wants_value`, `wants_acc` (3790-3794).
...
- The result type, when nothing asked: `Res<elem>` built from `type_decl("Res", parts, 1)` (3812-3821) — "a loop whose value nothing asked for still HAS one, and the `.match` on it needs its type". `result` is pre-set to `none_of(ret)`.
...
`lower_handle` (4406-4433) lowers `h.next()` / `h.break(v)`:
```python
if name == "next":
...
`range_value` (4355-4386): an array walks `base.zg_elems[counter]`; a named Range with no `at` walks its own index space (`counter` as `usize`) — "which is what makes `Range(0, 5)` a bare C for-loop"; otherwise `at(counter)` is called and a non-`Ok` result `goto`s the break label.
...
- **`==` on a named type** becomes the `Eq` impl's `eq` call, statically resolved (`eq_call`, 3185-3199); `!=` wraps it in `(!...)`. "a C `==` on a struct is not even legal C."
- **`consume x`** (`ex_Consume`, 2795-2801) is a no-op on the value; it calls `kill_drop` to clear the drop flag.
- **Struct literal** (`construct_type`, 2967-3002): `((T){ .zu_mx = ..., .zu_my = ... })`, in **declared field order**, positional args filled by index, `((T){0})` when nothing is supplied.
- **Fixed array / array literal** (5190-5207): `((T){ { e0, e1 } })` — note the doubled brace for the `zg_elems` member.
- **`ex_Record`** (5209) and **`ex_MetaCall`** (5222) are errors.
- `paren` (5326) only wraps when the code is neither an identifier nor already fully parenthesised.
- `int_literal` (5348-5367): unsigned always `((uint32_t)5ULL)`; `INT_MIN` spelled `((int32_t)(-2147483647LL - 1LL))`; otherwise `((int32_t)5LL)`. Gated by `tests/corpus/codegen/literal_boundaries_{signed,unsigned}.zen`.

## 6.9 RAII / drop / defer interleaving

`FnCtx.dscopes` / `sscopes` (2079-2082) follow the **emitted braces**, not `self.scopes` (which `inline_lambda` rewinds). `track_drop` (2147-2155) registers only `let` bindings whose type has a direct `Drop` impl; a name `consume`d anywhere program-wide gets an `int zg_liveN = 1;` flag (2152-2154, `consumed_names` 1333-1350 is deliberately a program-wide over-approximation: "Over-approximating costs an unread `int` on a frame; under-approximating costs a double free"). `exit_scope` (2193-2201) runs `zg_scope_run(&rec)` **before** any drop, then `unwind` walks the block's entries in reverse declaration order, skipping any binding whose C name appears in the block's value expression:
```python
...
`BANNER` (5451-5468), formatted with `C_STANDARD`:
```c
...
`INCLUDES` (5470-5478) — exactly six, always:
```c
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
...
`PRELUDE_TYPES` (5480-5492) — always:
```c
/* ---- runtime ---- */
...
`PRELUDE_SCOPE` (5535-5545) — only when `"scope" in self.needs`:
```c
...
`DEFER_RUNTIME` (5547-5580) — emitted at the end of `defer_section()`, *after* the per-site `zg_envN` structs and the `zg_defer_env` union:
```c
...
```
The union header (`defer_section` 1889-1892):
```python
...
`PRELUDE_PRINT` (5582-5593) — only when `"print" in self.needs`:
```c
...
There is **no allocator in the prelude**. `malloc`/`free` appear only in `lower_mem` (4183-4250), the lowering of the three bodyless `Mem` members:
...
Everything above that (Arena, Vec, String, Map) is ordinary Zen. `alloc.create<T>()` is lowered to `raw(sizeof(T), 16)` + `Ptr.to` (`lower_create` 4100-4127; the `16` is hardcoded ALIGN_MAX). `Ptr<T>`'s members are lowered to C directly (`ptr_method` 3302-3353): `read`→`p[i]`, `write`→`p[i] = v;`, `offset`→`(p + n)`, `back`→`(p - n)`, `bytes`→`(sizeof(T) * (size_t)n)`, `copy_from`→`memcpy(...)`, `is_null`→`(p == NULL)`, `to<U>`→`((U *)p)`.

## The `main` shim (`entry_point`, 2003-2044)
...
3. **`MAX_EXPR_DEPTH = 24` spilling** (2432-2438) plus `spill` along a folded spine (2857-2866). "per TESTING.md"; `tests/corpus/codegen/nesting_expr.zen`.
4. **`ex_Block` double-lowering** (4720-4734). The comment names the failure exactly: "a `.match` whose arms are ALL blocks therefore types as `()`: no result temporary is allocated and every arm's value is dropped, with no diagnostic and a zero in its place."
...
6. **`by_param_types` overload scoring** (3404-3457): "`digit(d: u64)` beside `digit(d: i64)` ... Resolving them by file order picks the wrong one half the time, and on `0 - (v % 10)` that is a spurious trap." Score is `(-bad, hits)`; numeric literals and lambdas contribute `None`.
...
10. **`_LVALUE` accepting `(*p).f.g`** (5308-5314): "Missing it copies the field into a temporary, so `self.entries.add(..)` grows a copy and the caller keeps the old one -- a silent wrong answer rather than a compile error."
...
12. **`peek`/`peek_block` roll back diagnostics** (3031-3042, 3052-3060) — plus five other ad-hoc `mark = len(self.diags)` / `del self.diags[mark:]` pairs at 1257/1287, 1380/1388, 3439/3441, 4276/4279, 4347/4351, 4523/4525. Speculative work must report nothing.
...
18. **`_block_body`'s trailing-`ExprStmt`-as-value rule** (2244-2253) and the `rty == UNIT` → `(void)(...)` rule (2269-2276): "`() { cleanup(x) }` emitting nothing at all."
19. **`emit_types` loops up to 8 times** (1902) because emitting a definition can register a new type; `topo` (1921-1944) breaks a by-value cycle by name *and* reports a diagnostic, so emission still terminates.
20. **`entry_point()` computed before `emit_types()`** (1845-1848): "`main` first: it can register one last type."
...
22. **`consumed_names` is program-wide** (1333-1340), not per-function, because bodies arrive inlined into their callers.
...
24. **`c_string` uses octal, never hex** (5428-5431) — partly to keep the determinism scanner's pointer-shaped-hex check quiet.
25. **`is_tparam` heuristic fallback** (3077-3085): `len(name) <= 2 and name[0].isupper()`. Explicitly acknowledged as insufficient — "`signed = <unsigned>(typedef: unsigned) unsigned` is generic in `unsigned`, and a shape heuristic cannot know that" — hence the `tparams` argument threaded through `unify`.
26. **`emit_call`'s bodyless-with-owner path** (3558-3574) emits `memset(&tmp, 0, sizeof tmp);` alongside the diagnostic "gen_c has no trait dispatch yet" — a placeholder value so the surrounding C still compiles.
...
28. **`console_sink` with `self = NULL`** (4689-4716) and `console_thunk` (1514-1543): the runtime *is* the Sink impl; printing a Display allocates nothing.
...
30. **`fmt_pieces`** (3920-3934): the *entire* format language — `{}` is a hole, "every other byte -- including a lone `{` and every `}` -- is literal."
31. **`module_named` three-tier search** (2634-2657): last path component, then a one-hop alias whose target names a module, then any component — "so a type never loses to a module".

### Code-quality oddities you should not replicate

- **`_lower_intrinsic` (4261-4272)**: the `if decl.owner == "Mem"` block sits *above* the triple-quoted string, so that string is not a docstring at all — it is a no-op expression statement in the middle of the function.
- **`lower_mem` line 4241**: `self.e.convert(value, None, mt) if False else value` — a permanently-dead conditional.
- **`lower_loop` line 3863**: `values = [(handle[0] and "0", handle)]` — `handle[0]` is the constant string `"loop"`, so this is an obfuscated `("0", handle)`.
- **`emit_fn` line 1766**: `"return (%s){0};" % self.ctype(ret) if ret[0] != "prim" else "return 0;"` — the `%` binds tighter than the ternary, so the format applies to the first branch only; it works, but it reads as a bug.
- **`range_bounds` line 4313** returns `("0", str(ty[1]))` — the array length as a *decimal string*, not a C expression, relying on the caller only interpolating it.
