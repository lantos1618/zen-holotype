...
def fold_comptime(files, namespace):
    ...
    for f in files.values():
        for d in f.decls:
            if isinstance(d, Fn) and d.body is not None and not d.extern:
                d.body = [_fold(s, namespace, d.scope) for s in d.body]
            elif isinstance(d, Impl):
                for m in d.methods:
                    if m.body is not None:
                        m.body = [_fold(s, namespace, m.scope) for s in m.body]
...
def _fold(e, namespace, scope):
    """Recurse an expr/stmt, replacing comptime(x) with its folded constant."""
    if isinstance(e, Call) and e.callee == "comptime":
        return _to_node(evaluate(e.args[0], namespace, scope), e.pos)
    if isinstance(e, Call):
        return replace(e, args=tuple(_fold(a, namespace, scope) for a in e.args))
    if isinstance(e, Bin):
        return replace(e, l=_fold(e.l, namespace, scope), r=_fold(e.r, namespace, scope))
    if isinstance(e, Not):
        return replace(e, operand=_fold(e.operand, namespace, scope))
    if isinstance(e, Field):
        return replace(e, obj=_fold(e.obj, namespace, scope))
    if isinstance(e, SliceLit):
        return replace(e, elems=tuple(_fold(x, namespace, scope) for x in e.elems))
    if isinstance(e, Index):
        return replace(e, seq=_fold(e.seq, namespace, scope), idx=_fold(e.idx, namespace, scope))
    if isinstance(e, StructLit):
        return replace(e, fields=tuple((n, _fold(v, namespace, scope)) for n, v in e.fields))
    if isinstance(e, EnumCtor):
        return replace(e, args=tuple(_fold(a, namespace, scope) for a in e.args))
    if isinstance(e, MethodCall):
        return replace(e, recv=_fold(e.recv, namespace, scope),
                       args=tuple(_fold(a, namespace, scope) for a in e.args))
    if isinstance(e, Match):
        arms = tuple(replace(a, body=_fold(a.body, namespace, scope)) for a in e.arms)
        return replace(e, subject=_fold(e.subject, namespace, scope), arms=arms)
    if isinstance(e, Let):
        return replace(e, value=_fold(e.value, namespace, scope))
    if isinstance(e, Assign):
        return replace(e, target=_fold(e.target, namespace, scope),
                       value=_fold(e.value, namespace, scope))
    if isinstance(e, While):
        return replace(e, cond=_fold(e.cond, namespace, scope),
                       body=tuple(_fold(s, namespace, scope) for s in e.body),
                       step=_fold(e.step, namespace, scope) if e.step is not None else None)
...
def _eval(e, env, namespace, scope, fuel):
    fuel[0] -= 1
    if fuel[0] <= 0:
    ...
    if isinstance(e, Var):
        ...
        return env[e.name]
    if isinstance(e, Not):
        return not _eval(e.operand, env, namespace, scope, fuel)
    if isinstance(e, Bin):
        return _binop(e.op, _eval(e.l, env, namespace, scope, fuel),
                      _eval(e.r, env, namespace, scope, fuel))
    if isinstance(e, Field):
        obj = _eval(e.obj, env, namespace, scope, fuel)
    ...
    if isinstance(e, Call):
        return _call(e, env, namespace, scope, fuel)
...
def _resolve_named(arg, scope, namespace, what):
    ...
    try:
        return namespace.walk(scope.get(arg.name, arg.name)).value
    except Exception:
        raise ComptimeErr(f"{what}: no such name '{arg.name}'")


def _bi_reflect(e, env, namespace, scope, fuel):
    node = _resolve_named(e.args[0], scope, namespace, "reflect")
    if not isinstance(node, (Struct, EnumDecl)):
        raise ComptimeErr(f"reflect: '{e.args[0].name}' is not a type")
    return node


def _bi_reflect_trait(e, env, namespace, scope, fuel):
    node = _resolve_named(e.args[0], scope, namespace, "reflect_trait")
    if not isinstance(node, TraitDecl):
        raise ComptimeErr(f"reflect_trait: '{e.args[0].name}' is not a trait")
    return node


def _bi_trait_method_name(e, env, namespace, scope, fuel):
    t = _eval(e.args[0], env, namespace, scope, fuel)
...
def _bi_trait_method_count(e, env, namespace, scope, fuel):
    t = _eval(e.args[0], env, namespace, scope, fuel)
    if not isinstance(t, TraitDecl):
...
def _bi_trait_method_name_at(e, env, namespace, scope, fuel):
    t = _eval(e.args[0], env, namespace, scope, fuel)
    i = _eval(e.args[1], env, namespace, scope, fuel)
    if not isinstance(t, TraitDecl):
        raise ComptimeErr("trait_method_name_at: argument is not a trait")
    return t.methods[i].name


def _bi_name_of(e, env, namespace, scope, fuel):
    return _eval(e.args[0], env, namespace, scope, fuel).name


def _bi_field_count(e, env, namespace, scope, fuel):
    t = _eval(e.args[0], env, namespace, scope, fuel)
...
def _bi_field_name_at(e, env, namespace, scope, fuel):
    t = _eval(e.args[0], env, namespace, scope, fuel)
    i = _eval(e.args[1], env, namespace, scope, fuel)
...
def _bi_variant_count(e, env, namespace, scope, fuel):
    t = _eval(e.args[0], env, namespace, scope, fuel)
...
def _bi_variant_name_at(e, env, namespace, scope, fuel):
    t = _eval(e.args[0], env, namespace, scope, fuel)
    i = _eval(e.args[1], env, namespace, scope, fuel)
    if not isinstance(t, EnumDecl):
...
def _bi_variant_has_payload(e, env, namespace, scope, fuel):
    t = _eval(e.args[0], env, namespace, scope, fuel)
    i = _eval(e.args[1], env, namespace, scope, fuel)
    if not isinstance(t, EnumDecl):
...
def _bi_concat(e, env, namespace, scope, fuel):
    return str(_eval(e.args[0], env, namespace, scope, fuel)) + \
           str(_eval(e.args[1], env, namespace, scope, fuel))
...
def _call(e, env, namespace, scope, fuel):
    if e.callee in _BUILTINS:                     # reified-AST surface, before everything else
        return _BUILTINS[e.callee](e, env, namespace, scope, fuel)
    ...
    fn = namespace.walk(target).value
    ...
    return _block(fn.body, fenv, namespace, fn.scope, fuel)


def _block(stmts, env, namespace, scope, fuel):
    env, val = dict(env), None
    for s in stmts:
        if isinstance(s, Let):
            env[s.name] = _eval(s.value, env, namespace, scope, fuel)
        else:
            val = _eval(s, env, namespace, scope, fuel)
    return val


def _match(e, env, namespace, scope, fuel):
    subj = _eval(e.subject, env, namespace, scope, fuel)
